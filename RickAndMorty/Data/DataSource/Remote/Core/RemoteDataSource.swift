...
struct RemoteDataSource {
    ...
    static func runAndDecodeDataAndHeaders<D: Decodable, H: Decodable>(_ urlRequest: URLRequest) -> AnyPublisher<BaseEntityWithHeaders<D, H>, DataError> {
        URLSession.shared
            .dataTaskPublisher(for: urlRequest)
            .tryMap { data, response -> BaseEntityWithHeaders<D, H> in
                try processResponse(response, data: data)
                ...
                do {
                    ...
                    return BaseEntityWithHeaders(data: decodedData, headers: decodedHeaders)
                } catch {
                    throw DataError.decoding
                }
            }
